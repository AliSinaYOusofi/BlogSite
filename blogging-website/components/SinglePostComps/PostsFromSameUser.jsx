...
export default function PostsFromSameUser({id, date, content, title, username}) { // from the postId we can get poster
    ...
    return (
        <div className="md:w-fit w-[95%] mx-auto p-4 bg-[#f5f5f5] rounded-md ml-2 mt-4 sticky top-0" key={id}>
            ...
            <Link onClick={moveToTopOfDocument} href={{ pathname:"/single_post_view", query: {post: id} }} className="text-blue-300 transition-all duration-300 hover:underline text-sm" id={id}>{username ? "Read more ⟶" : null}</Link> 
        </div>
    )
}
