...
export default function UserPosts({content, title, username, profileUrl, date, id}) {
    ...
    return (
        <div id={id} className="max-w-2xl px-8 py-4 mx-auto  rounded-lg shadow-md bg-gray-800 mt-10">
            <div className="flex items-center justify-between">
                <span className="text-sm font-light text-gray-600 dark:text-gray-400">{date}</span> 
            </div> 
            <div className="mt-2">
                <Link href="https://stackdiary.com/" className="overflow-ellipsis line-clamp-1 text-2xl font-bold text-white hover:text-gray-600 dark:hover:text-gray-200 hover:underline">{title}</Link> 
            ...
            <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center">
                    ...
                    <Link href="#linkToUserProfile" className="font-bold text-gray-700 cursor-pointer dark:text-gray-200">{username}</Link>
                </div>
            </div>
        </div>
    )
