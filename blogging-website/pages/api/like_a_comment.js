...
export default async function handler(req, res) {
    ...
    const alreadyLiked = await commentLikes.findOne({"who": emailOfLiker, "commentId": commentId}, {"loves": 1}); // if loves is one then he/she like
    let insertedLikes = 0;
    ...
    try {
        if (alreadyLiked) {
            ...
            await commentLikes.updateOne({"who": emailOfLiker, "commentId": commentId}, { $inc: {"loves": insertedLikes}});
            return res.status(200).json({message: "loved"});
        }

        await commentLikes.insertMany([{"who": emailOfLiker, "commentId": commentId, "loves": 1}]);
